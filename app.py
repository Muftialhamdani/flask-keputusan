...
class TeoriKeputusan:
    ...
    def kondisi_ketidakpastian(self, problem_type='minimization'):
        """
        Pengambilan keputusan pada kondisi ketidakpastian
        
        Args:
            problem_type (str): 'minimization' untuk biaya, 'maximization' untuk profit
        """
        results = {}
        
        # 1. MAXIMAX (Optimistik)
        if problem_type == 'minimization':
            # Untuk minimisasi: cari nilai minimum dari setiap alternatif
            optimal_values = np.min(self.payoff_matrix, axis=1)
            # Pilih yang terkecil dari nilai-nilai minimum
            maximax_index = np.argmin(optimal_values)
        else:
            # Untuk maksimisasi: cari nilai maksimum dari setiap alternatif  
            optimal_values = np.max(self.payoff_matrix, axis=1)
            # Pilih yang terbesar dari nilai-nilai maksimum
            maximax_index = np.argmax(optimal_values)
        
        maximax_value = optimal_values[maximax_index]
        
        results['maximax'] = {
            'alternatif': self.alternatives[maximax_index],
            'nilai': float(maximax_value),
            'detail': {alt: float(val) for alt, val in zip(self.alternatives, optimal_values)},
            'tipe': f'{"Minimum" if problem_type == "minimization" else "Maximum"} values per alternative'
        }
        
        # 2. MAXIMIN (Pesimistik) 
        if problem_type == 'minimization':
            # Untuk minimisasi: cari nilai maksimum dari setiap alternatif
            worst_values = np.max(self.payoff_matrix, axis=1)
            # Pilih yang terkecil dari nilai-nilai maksimum (minimax)
            maximin_index = np.argmin(worst_values)
        else:
            # Untuk maksimisasi: cari nilai minimum dari setiap alternatif
            worst_values = np.min(self.payoff_matrix, axis=1)
            # Pilih yang terbesar dari nilai-nilai minimum
            maximin_index = np.argmax(worst_values)
        
        maximin_value = worst_values[maximin_index]
        
        results['maximin'] = {
            'alternatif': self.alternatives[maximin_index],
            'nilai': float(maximin_value),
            'detail': {alt: float(val) for alt, val in zip(self.alternatives, worst_values)},
            'tipe': f'{"Maximum" if problem_type == "minimization" else "Minimum"} values per alternative'
        }
        
        # 3. SAMA RATA (Equally Likely / Laplace)
        avg_values = np.mean(self.payoff_matrix, axis=1)
        
        if problem_type == 'minimization':
            # Pilih rata-rata terkecil
            sama_rata_index = np.argmin(avg_values)
        else:
            # Pilih rata-rata terbesar
        ...
        results['sama_rata'] = {
            ...
            'tipe': 'Average values per alternative'
        }
        
        return results
        
    def kondisi_beresiko(self, probabilities, problem_type='minimization'):
        ...
        # Tentukan keputusan terbaik berdasarkan problem type
        if problem_type == 'minimization':
            best_index = np.argmin(expected_values)  # Pilih EV terkecil
        else:
            best_index = np.argmax(expected_values)  # Pilih EV terbesar
    ...
    def kondisi_pasti(self, state_index, problem_type='minimization'):
        ...
        if 0 <= state_index < len(self.states):
            ...
            if problem_type == 'minimization':
                best_index = np.argmin(payoffs)  # Pilih payoff terkecil
            else:
                best_index = np.argmax(payoffs)  # Pilih payoff terbesar
    ...
    def analisis_sensitivitas(self, problem_type='minimization'):
        ...
        for prob in prob_range:
            ...
            # Pilih terbaik berdasarkan problem type
            if problem_type == 'minimization':
                best_index = np.argmin(expected_values)
            else:
...
def kondisi_ketidakpastian_route():
    try:
        if decision.payoff_matrix is None:
            return jsonify({'error': 'Matriks payoff belum diinput'})
        
        # Menggunakan metod kondisi_ketidakpastian dengan parameter minimization
        results = decision.kondisi_ketidakpastian(problem_type='minimization')
...
def kondisi_beresiko():
    try:
        ...
        # Menggunakan parameter minimization untuk masalah biaya
        results = decision.kondisi_beresiko(probabilities, problem_type='minimization')
        return jsonify(results)
...
def kondisi_pasti():
    try:
        ...
        # Menggunakan parameter minimization untuk masalah biaya
        results = decision.kondisi_pasti(state_index, problem_type='minimization')
        return jsonify(results)
...
def visualisasi():
    try:
        ...
        if data.get('include_ketidakpastian'):
            hasil_ketidakpastian = decision.kondisi_ketidakpastian(problem_type='minimization')
        
        if data.get('include_beresiko') and data.get('probabilities'):
            hasil_beresiko = decision.kondisi_beresiko(data['probabilities'], problem_type='minimization')
...
def analisis_sensitivitas():
    try:
        ...
        results = decision.analisis_sensitivitas(problem_type='minimization')
        return jsonify(results)
